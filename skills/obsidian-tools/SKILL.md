...
```text
                     ┌─── web search 补值 ───┐
                    │                       ↓
raw → compile → wiki pages → ask / output / lint
       ↑           │              │
       │           │              ↓
...
| `../remnote-card-designer/SKILL.md` | RemNote 卡片设计约定 | ask（晋升时调用） |

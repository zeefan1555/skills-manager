...
在知识库中检索已有知识。用户已知知识库里有相关内容，需要找到并组织回答。根据问题复杂度自动选择 shallow（关键词搜索）或 deep（语义推理）模式。检索遵循 4 层层级：L1 index.md → L2 concepts → L3 summaries → L4 raw。
...
3. append 到 `wiki/log.md`，格式 `[{date}] query | {question}`

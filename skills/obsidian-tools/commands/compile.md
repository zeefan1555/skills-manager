...
> **引用规范**：`references/note-format.md`（Summary / Concept 格式、promote 状态机）、`references/obsidian-cli.md`（CLI 语法）、`references/markdown.md`（正文格式）
...
把 `raw/` 中 `status: pending` 的内容增量编译成 wiki 页面，并更新受影响页面：
...
并把原 raw 的 `status` 更新为 `compiled`。

## 模式选择

| 模式 | 触发 | 行为 |
|---|---|---|
| **增量**（默认） | `compile` | 只处理 `status: pending` 的 raw |
| **定向更新** | `compile --target <path-or-slug>` | 只更新某个 concept / topic / synthesis 及其依赖页面 |
| **全量重建** | `compile --rebuild` | 重新编译所有 raw，重建全部索引 |
| **批量** | `compile --batch` | pending > 20 时分批处理，每批 10 篇 |
...
1. 读取 `_kb_meta/compile_log.md`
2. 扫描 `raw/` 下所有候选笔记
   - 优先 `obsidian files folder="raw" ext=md`（见 `references/obsidian-cli.md`）
3. 找出 `status: pending` 的笔记（增量模式）或全部笔记（全量模式）
...
8. 把 raw 标为 `compiled`，写入 `compiled_at`
9. 更新 `_kb_meta/compile_log.md` 和 `wiki/log.md`
...
- 哪些 raw 从 `pending` 变成了 `compiled`
- 建议下一步操作（如 `search`、`lint`、`status`）
...
- 如果 pending 数量过多（> 20），自动切换 batch 模式，每批 10 篇并报告进度

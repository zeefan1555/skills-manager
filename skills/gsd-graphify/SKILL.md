...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node $HOME/.trae/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
```
PREFLIGHT=$(node "$HOME/.trae/get-shit-done/bin/gsd-tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: $HOME/.trae/get-shit-done/bin/gsd-tools.cjs
